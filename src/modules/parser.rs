...
//Error handling
use crate::modules::errorhandler::{handle_error, ErrorReason, ErrorDescription, ErrorType};

pub fn parse_source(src:String, node_global:Node, file_path:String) -> Rc<RefCell<Node>>{
    ...
    for (linenum, line) in src.lines().enumerate() {
        ...
        if if is_template {!line_val.chars().all(char::is_whitespace)} else {true}
        {
			...
			for (colnum, origin_char) in line_val.chars().enumerate()
			{
				if is_template {
							...
							if !(last_char != '\\' && origin_char == '"')
							{
								if origin_char == '\n' {
									handle_error(
										...
										"Insert or remove a '\"' to close the string.",
										ErrorType::Syntax,
										format!("{}",linenum.clone() + 1).as_str(),
										format!("{}",colnum.clone()).as_str(),
										file_path.clone().as_str()
